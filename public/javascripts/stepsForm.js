...
;( function( window ) {
	...
	stepsForm.prototype._nextQuestion = function() {
		...
		var self = this,
			onEndTransitionFn = function( ev ) {
				if( support.transitions && (!self || !self.retrying) ) {
		...
		if( support.transitions && !this.retrying ) {
	...
	stepsForm.prototype._submit = function() {
		var that = this;
		this.options.onSubmit( this.el, function() {
			// error callback, set things back up for another submit
			--that.current;
			that.retrying = true;
		} );
	}
